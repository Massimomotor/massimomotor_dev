...
release_metadata = {"name":"SuiteCommerce Advanced Aconcagua","prodbundle_id":"228165","baselabel":"SCA_Aconcagua","version":"Aconcagua Release 2","datelabel":"2018.06.19","buildno":"2"}
/**
    ...
    return SCModel.extend({
        ...
        categoryColumns: {
            boolean: [
                ...
                'isinactive',
                'isprimaryurl',
                'custrecord_tt_use_custom_template'
            ],
            sideMenu: {
            ...
            mapping: {
                ...
                'custrecord_tt_use_custom_template': 'custrecord_tt_use_custom_template'
            }
        },
...
define('CMSOnSecure.Model', [
    ...
    return SCModel.extend({
        ...
        get: function () {
            return {
                'live_template_path': '/CMS/Development/Published/Templates/cms-templates-015ace7a1fd1.js',
                ...
                'comp_id': '4521019',
                'site_id': 2,
                'system_domain': 'https://system.na2.netsuite.com/',
                'commerce_categories_enabled': true
            };
        }
    });
});
define('CMSOnSecure.ServiceController', [
    'ServiceController',
    'Application',
    'CMSOnSecure.Model'
], function (ServiceController, Application, CMSOnSecureModel) {
    'use strict';
    return ServiceController.extend({
        ...
        create: function (data) {
            ...
            try {
                ...
                if (responseCode === 200 || responseCode === 302 || responseCode === 201 || responseCode === 404) {
                    return { successMessage: 'Your request has been submitted, we will get back to you within 24 hours.' };
                }
                out.code = 'OK';
            } catch (e) {
...
define('Categories.Model.Extension', [
    'Categories.Model',
    'SC.Model',
    'SC.Models.Init',
    'Application',
    'Configuration',
    'underscore'
], function (CategoriesModel, SCModel, ModelsInit, Application, Configuration, _) {
    'use strict';
    _.extend(CategoriesModel, {
        getCategories: function (parenturl, categoryIds, columns) {
            ...
            if (parenturl) {
                ...
                if (i) {
                    ...
                    categoriesData = this.getCategoriesData(parenturl, categoryids, columns);
                    var processColumn = function (column) {
                        if (column === 'custrecord_tt_use_custom_template') {
                            category[column] = categoryData[column] || category[column];
                        } else {
                            category[column] = category[column] || categoryData[column];
                        }
                    };
        ...
        getCategoryTree: function (level) {
            ...
            var self = this, result = [], siteId = ModelsInit.session.getSiteSettings(['siteid']).siteid, filters = [
                    ...
                    new nlobjSearchColumn('nestlevel'),
                    new nlobjSearchColumn('idpath').setSort(false),
                    new nlobjSearchColumn('thumbnailurl')
                ], bag = {}, overrides = {}, columns = this.getColumns('menu'), toSubCategory = this.categoryColumns.mapping;
            _.each(columns, function (column) {
                searchColumns.push(new nlobjSearchColumn(column));
                toSubCategory[column] && searchColumns.push(new nlobjSearchColumn(toSubCategory[column]));
            ...
            Application.getAllSearchResults('commercecategory', filters, searchColumns).forEach(function (line) {
                ...
                if (!bag[idPath]) {
                    ...
                    current.fullurl = line.getValue('fullurl');
                    current.level = line.getValue('nestlevel');
                    current.thumbnailurl = line.getValue('thumbnailurl');
                    var parentIdPathArr = idPath.split('|');
                    if (parentIdPathArr.length > 3) {
                        parentIdPathArr.splice(parentIdPathArr.length - 2, 1);
                    } else {
        ...
        getCategoriesData: function (parenturl, categoryIds, columns) {
            ...
            try {
                ...
                _.each(result, function (line) {
                    var catData = {};
                    _.each(columns, function (column) {
                        catData[column] = line.getValue(column);
                    });
                    allCategories.push(catData);
                });
                var primaryCat = _.find(allCategories, function (currentCat) {
                    return currentCat.isprimaryurl == 'T';
                });
                primaryCat && _.each(categories, function (currentCat) {
                    currentCat.canonical = primaryCat.fullurl;
                });
            } catch (e) {
                nlapiLogExecution('ERROR', 'Error on the canonical patch', e);
            }
            return categories;
        }
    });
    ...
    return SCModel.extend({
        ...
        create: function (data) {
            ...
            try {
                ...
                var url = nlapiResolveURL('SUITELET', 'customscript_tt_handle_contact_us', 'customdeploy_tt_handle_contact_us', true);
                nlapiLogExecution('DEBUG', 'URL', url);
                var response = nlapiRequestURL(url, request, null, 'POST');
                var responseCode = parseInt(response.getCode(), 10);
                if (responseCode === 200 || responseCode === 302 || responseCode === 201 || responseCode === 404) {
                    return { successMessage: 'Your request has been submitted, we will get back to you within 24 hours.' };
                }
...
define('Warranty.ServiceController', [
    'ServiceController',
    'Application',
    'Warranty.Model'
], function (ServiceController, Application, WarrantyModel) {
    'use strict';
    return ServiceController.extend({
        name: 'Warranty.ServiceController',
        post: function () {
            nlapiLogExecution('DEBUG', 'WARRANTY DATA', JSON.stringify(this.data));
            this.sendContent(WarrantyModel.create(this.data), { 'status': 201 });
        }
    });
});
define('SCA', [
    ...
    'Categories.Model.Extension',
    'Warranty.Model',
    'Warranty.ServiceController'
], function () {
});
